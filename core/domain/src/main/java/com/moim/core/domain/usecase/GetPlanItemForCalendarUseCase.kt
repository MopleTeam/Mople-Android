...
class GetPlanItemForCalendarUseCase @Inject constructor(
    ...
    suspend operator fun invoke(date: String): List<PlanItem> =
        withContext(ioDispatcher) {
            val planAndReview = planRepository.getPlansForCalendar(date)
            planAndReview.plans.map(Plan::asPlanItem) + planAndReview.reviews.map(Review::asPlanItem)
        }
}

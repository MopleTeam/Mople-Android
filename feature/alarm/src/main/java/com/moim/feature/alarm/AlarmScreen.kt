...
fun AlarmRoute(
    ...
    viewModel.collectSideEffect { sideEffect ->
        when (sideEffect) {
            is AlarmSideEffect.NavigateToBack -> navigateToBack()
            is AlarmSideEffect.NavigateToMeetingDetail -> navigateToMeetingDetail(sideEffect.meetingId)
            is AlarmSideEffect.NavigateToPlanDetail -> navigateToPlanDetail(sideEffect.viewIdType)
        }
    }

    LaunchedEffect(uiState.isSuccess) {
        if (uiState.isSuccess) {
            viewModel.onIntent(AlarmIntent.NotificationCountUpdate)
        }
    }
...
private fun AlarmScreen(
    ...
    Column(
        ...
        MoimTopAppbar(
            ...
            onClickNavigate = { onIntent(AlarmIntent.BackClick) },
        )
        ...
        Box(
            ...
            FadeAnimatedVisibility(uiState.isError) {
                ErrorScreen {
                    onIntent(AlarmIntent.RefreshClick)
                }
            }

            FadeAnimatedVisibility(uiState.isSuccess && uiState.alarms.isNotEmpty()) {
                PaginationEffect(
                    listState = listState,
                    threshold = 3,
                    enabled = !paging.isLast && !paging.isErrorFooter,
                    onNext = { onIntent(AlarmIntent.NextPageLoad) },
                )
            ...
            FadeAnimatedVisibility(uiState.isSuccess && uiState.alarms.isEmpty()) {
                AlarmEmptyScreen()
            }
        }
    }
